...
OLLAMA_MODEL = "qwen2.5-coder:3b"  # Lightweight: ~2GB, good reasoning
...
class OllamaLLM:
    """Simple wrapper for ollama that matches LangChain ChatModel interface."""
    ...
    def invoke(self, messages: list) -> AIMessage:
        """Invoke the LLM with messages, returning AIMessage."""
        # Convert LangChain messages to ollama format
        # Note: Use 'user' role for all messages as qwen3-coder doesn't handle 'system' well
        ollama_messages = []
        for msg in messages:
        ...
        if not content:
            print(f"⚠️ Warning: Empty response from {self.model}", flush=True)
...
class PlanTool(AgentTool):
    ...
    def execute(
        self, goal: str, skill_name: str, iteration: int = 1, frameworks: list = None
    ) -> dict:
        """
...
class WriteTool(AgentTool):
    ...
    def execute(self, skill_name: str, code: str) -> dict:
        ...
        try:
            ...
            return {"success": False, "error": str(e), "message": f"Write failed: {e}"}
...
class TestTool(AgentTool):
    ...
    def execute(self, skill_name: str, code: str) -> dict:
        ...
        if result["success"]:
            ...
            return {"success": True, "output": output, "message": "Test passed"}
        else:
            output = f"ERROR:\n{result['stderr']}\n\nOUTPUT:\n{result['stdout']}"

            # Log failure to memory
            self.memory.log_failure(
                skill=skill_name, error=result["stderr"], code_snippet=code[:500]
            )
...
Be strict: only mark as SUCCESS if output shows clear success."""
        ...
        return {"success": is_success, "analysis": analysis, "message": analysis}
    ...
    def execute(self, operation: str, **kwargs) -> dict:
        ...
        try:
            if operation == "add_skill":
                ...
                return {
                    "success": True,
                    "message": f"Skill {kwargs['skill_name']} added to memory",
                }
                ...
                failures = self.memory.get_relevant_failures(
                    kwargs["skill_name"], limit=kwargs.get("limit", 5)
                )
            ...
            for idx, step in enumerate(steps):
                ...
                def make_node_fn(step_desc, step_idx):
                    ...
                    def node_fn(state: PlanState) -> PlanState:
                        print(
                            f"  Executing step {step_idx + 1}/{len(steps)}: {step_desc.get('name', 'unnamed')}"
                        )
...
class LLMController:
    ...
    def decide_next_action(
        self, goal: str, skill_name: str, iteration: int, history: list
    ) -> dict:
        ...
        prompt = f"""You are an autonomous agent. Follow this EXACT workflow to create skills.
...
1. plan_skill - Generate code (required first step)
2. write_skill - Save code to file
3. test_skill - Run and verify the code
4. COMPLETE - Only after test passes

CURRENT STATUS: {history_text if history else "Starting - need to plan first"}

Respond in EXACT format:
DECISION: <brief reason>
ACTION: <plan_skill OR write_skill OR test_skill OR COMPLETE>
...
"""
        ...
        try:
            ...
            return {"decision": decision, "action": action, "params": params}
            ...
            self.memory.log_failure(
                skill="llm_controller", error=error_msg, code_snippet=content[:500]
            )
...
Be strict: only mark as SUCCESS if output shows clear success."""
    ...
    def router(
        ...
        "plan_skill", "write_skill", "test_skill", "analyze_results", "__end__"
    ]:
    ...
    def build_graph(self) -> StateGraph:
        ...
        workflow.add_conditional_edges("plan_skill", self.router)
        workflow.add_conditional_edges("write_skill", self.router)
        workflow.add_conditional_edges("test_skill", self.router)
        workflow.add_conditional_edges("analyze_results", self.router)
    ...
    def run_llm_central(self, goal: str, skill_name: str):
        ...
        while iteration <= MAX_ITERATIONS:
            ...
            decision = self.controller.decide_next_action(
                goal, skill_name, iteration, history
            )
            ...
            # Auto-inject required params for plan_skill if missing
            if action == "plan_skill":
            ...
            # Auto-inject required params for write_skill if missing
            if action == "write_skill":
                if "skill_name" not in params:
                    params["skill_name"] = skill_name
                if not params.get("code") and skill_code:
                    params["code"] = skill_code
                # Filter out extra params
                params = {
                    k: v for k, v in params.items() if k in ["skill_name", "code"]
                }

            # Auto-inject required params for test_skill if missing
            if action == "test_skill":
                if "skill_name" not in params:
                    params["skill_name"] = skill_name
                if not params.get("code") and skill_code:
                    params["code"] = skill_code
                # Filter out extra params that test_skill doesn't accept
                params = {
                    k: v for k, v in params.items() if k in ["skill_name", "code"]
                }

            # Auto-inject required params for analyze_results if missing
            if action == "analyze_results":
                if "skill_name" not in params:
                    params["skill_name"] = skill_name
                # Filter out extra params
                params = {k: v for k, v in params.items() if k in ["skill_name"]}
            ...
            if action == "COMPLETE":
                ...
                self.memory.add_skill(
                    name=skill_name, description=goal, status="working"
                )
                return True
            ...
            if action == "FAILED":
                ...
                self.memory.add_skill(
                    name=skill_name, description=goal, status="failed"
                )
                return False
        ...
        self.memory.add_skill(name=skill_name, description=goal, status="failed")
        return False
...
def main():
    ...
    while i < len(args):
        ...
        if arg in ["--mode", "-m"] and i + 1 < len(args):
            mode = args[i + 1]
            i += 2
        elif arg in ["--directive", "-d"] and i + 1 < len(args):
            directive = args[i + 1]
            i += 2
        elif arg == "--graph":
            mode = "graph"
...
╚═══════════════════════════════════════════════════════════════╝
    ...
    # Non-interactive mode: run directive and exit
    if directive:
        print(f"\n▶ Running directive: {directive}\n")
        agent.run(directive)
        print("\n✓ Directive completed. Exiting (non-interactive mode).")
        return
    ...
    try:
        ...
        if not os.isatty(sys.stdin.fileno()):
            print("⚠️  Running in non-terminal mode. Use -d <directive> to run a task.")
            print(
                '   Example: python3 autonomous_agent.py -d "Create a hello world function"'
            )
            sys.exit(0)
    except:
        pass
